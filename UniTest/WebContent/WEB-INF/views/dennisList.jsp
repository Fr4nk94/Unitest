					...
					<c:forEach var="exam" items="${examInList}">
						<tr examID="${ exam.examID }" class="odd gradeX">
							...
							<td id="putButton" class="center">
								...
								<c:choose>
									<c:when test="${not exam.available}">
										<input id="avalButton-${ exam.examID }"
											onclick="changeAvailability(${ exam.examID })" type="button"
											class="btn btn-warning" value="make available"></input>
									</c:when>

									<c:otherwise>
										<input id="avalButton-${ exam.examID }"
											onclick="changeAvailability(${ exam.examID })" type="button"
											class="btn btn-primary" value="make unavailable"></input>
									</c:otherwise>
								</c:choose>
							</td>
						</tr>
